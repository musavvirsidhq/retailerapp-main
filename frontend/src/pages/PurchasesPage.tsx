...
function PurchasesPage() {
  ...
  async function loadAll() {
    try {
      setLoading(true)
      const [p, f, pr] = await Promise.all([listPurchases(), listFactories(), listProducts()])
      setPurchases(p)
      setFactories(f)
      setProducts(pr)
    } catch {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="font-display font-bold text-2xl mb-1">Purchases</h1>
      ...
      <form onSubmit={handleSubmit} className="card p-6 mb-8">
        <div className="grid grid-cols-3 gap-3 mb-5">
          ...
          <input className="input-field" placeholder="Invoice number" value={invoiceNo} onChange={(e) => setInvoiceNo(e.target.value)} />
          <input className="input-field mono-num" placeholder="Amount paid now" type="number" step="0.01" value={amountPaid} onChange={(e) => setAmountPaid(e.target.value)} />
        </div>
        ...
        <div className="flex justify-between items-center mt-6 pt-5 border-t border-line">
          ...
          <button type="submit" className="btn-accent">Save Purchase</button>
        </div>
      </form>
